...
const page = async ({ params }) => {
    const singleNews = await fetch(`${base_url}/api/v2/detail/${params.id}`)
    const singleNewsData = await singleNews.json();
    ...
    return (
        <div className="container mx-auto ps-4">
            <h1 className="md:text-3xl text-xl font-semibold mt-8">{singleNewsData.title}</h1>
            <div className='flex gap-4 items-center mt-4'>
                <Image
                    className='w-12 h-12 rounded-full'
                    width={48} height={48}
                    src={`${process.env.BASE_URL}/${singleNewsData.featured_image}`}
                    alt='top Image'
                />
                <div>
                    <Link href={"/authorPage/1"}>
                        <h1 className='md:text-lg text-sm font-semibold'>{singleNewsData.title}</h1>
                    </Link>
                    <p>{singleNewsData.author}</p>
                </div>
            </div>
            <div className='grid grid-cols-12 gap-6'>
                <div className='md:col-span-8 col-span-12 mr-3'>
                    <Image
                        ...
                        src={`${process.env.BASE_URL}/${singleNewsData.featured_image}`}
                        alt={singleNewsData.featured_image_caption}
                    />
                    <p className='text-lg mb-4' dangerouslySetInnerHTML={{ __html: singleNewsData.content }} />
                </div>
                <div className='md:col-span-4 col-span-12 mt-4 mr-3 mb-8 md:mb-0'>
                    ...
                    <div className='bg-white px-2 py-1 border-2 border-x-slate-500 border-b-slate-500'>
                        {
                            todaysNewsData?.slice(0, 6).map((tn) =>
